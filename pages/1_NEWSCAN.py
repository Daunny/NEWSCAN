...
# 타이틀
st.title(":desktop_computer: NEWSCAN")

# 뉴스 일자 필터링
st.subheader("뉴스 일자 선택")
start_time = st.date_input(
    'Select date',
    value=(datetime(2023, 5, 14), datetime(2023, 5, 28)),
...
ab_tfidf_matrix = tfidf.fit_transform(df3)
...
with col01:
    ...
    "Select 키워드 to View", options = k_men, default = ['삼성'],
    label_visibility='collapsed',
    max_selections=1)
...
with col02:
    ...
    "Select 언론사 to View", options = accounts, default = ['조선일보','중앙일보','한겨레','한국경제','동아일보', '머니투데이', '파이낸셜뉴스','헤럴드경제'] ,
    label_visibility='collapsed')
...
df['proba'] = df['proba'].map(lambda x : (x-0.5)*2)
df['Sim_score'] = df['Sim_score'].map(lambda x : (x-0.5)*2)
df['fishing_score'] = df['fishing_score'].map(lambda x : (x-0.5)*2)
...
st.subheader(f':date:{y}년 {m}월 {d0} ~ {m}월 {d1}일 ')
...
with col1:
    ...
    if 'Senti_label' in temp_df.columns:
        X = temp_df['Senti_label']
    else:
        X = temp_df['count']
    wdges, labels, autopct = plt.pie(X, labels=temp_df.index, startangle=90,
...
with col2:
    ...
    st.markdown('필터링 된 경제 기사에서 가장 많이 언급된 키워드를 보여줍니다.')
    st.caption('언급량은 키워드의 크기에 비례합니다.')
    ...
    with col_c:
        ...
        # 벡터화
        tfidf = TfidfVectorizer(min_df=10, max_features=30)
...
gb = GridOptionsBuilder.from_dataframe(df[['일자', '제목', '긍부정 여부', '제목-본문 일치도(%)', '피싱기사 여부',]])
# configure selection
...
st.caption('- 긍부정 여부 : 분류 결과 가장 높은 확률(probablity)을 가진 라벨값(**중립/긍정/부정**)을 제시  \n '
           '- 제목 본문 일치도 : 제목과 본문의 일치 정도를 퍼센트로 제시. 30% 미만인 경우,  :arrow_right: 의도적 제목 왜곡형으로 분류  \n'
           '- 피싱기사 여부 : 피싱 확률이 0.5 이상인 경우, 🐟피싱 의심 \n'
           '- 피싱기사 종류 : 자동생산 기사, 속어/줄임말 사용형, 상품/서비스 판매정보 노출 광고형, 의도적 상황 왜곡/전환형')
...
if len(selected_rows) != 0:
    ...
    if st.button('기사 원문보기'):
        webbrowser.open_new_tab(selected_rows[0]['URL'])
